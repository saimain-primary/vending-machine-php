...
<template>
    <div class="min-h-screen bg-[oklch(0.965_0.005_70)]" @click="dropdownOpen = false">
            ...
            <div class="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
                ...
                <div v-if="authenticatedUser" class="relative" @click.stop>
                    ...
                    >
                        ...
                        <svg
                            :class="['h-4 w-4 text-[oklch(0.55_0.006_260)] transition-transform duration-150', dropdownOpen && 'rotate-180']"
                    ...
                    >
                        <div
                            v-if="dropdownOpen"
                            class="absolute right-0 top-full mt-1 w-44 border border-[oklch(0.88_0.006_70)] bg-white py-1 shadow-lg"
                        >
                            <template v-if="!isAdmin">
                                ...
                                >
                                    <svg class="h-4 w-4 text-[oklch(0.55_0.006_260)]" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                                    </svg>
                                    Order History
                                </Link>
                                <div class="my-1 border-t border-[oklch(0.92_0.005_70)]" />
                            </template>
                            <button
        ...
        <main class="mx-auto max-w-6xl px-6 py-8">
            <h1 v-if="title" class="mb-6 text-2xl font-bold text-[oklch(0.15_0.01_260)]">{{ title }}</h1>
            <slot />
        </main>
    </div>
